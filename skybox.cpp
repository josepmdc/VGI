...
unsigned int loadCubemap(std::vector<std::string> faces) {
    ...
    for (unsigned int i = 0; i < faces.size(); i++) {
        //nrChannel
        unsigned char* data = stbi_load(faces[i].c_str(), &width, &height, 0, 0);
...
void OnVistaSkyBox() {
    ...
    // 
      //Posar identificar skC !!  sk_programID
      //Gl
      //Carrego i dona identificador
      //De cada shader crear una clase nova. 
      //Mateixos uniform noms a tots els shaders .txt
